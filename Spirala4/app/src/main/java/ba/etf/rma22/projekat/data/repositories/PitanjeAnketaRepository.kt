        ...
        return withContext(Dispatchers.IO) {
            if (NetworkUtil.isDeviceConnected) {
                ...
                if (response.isSuccessful) {
                    val pitanja = response.body()!!
                    pitanja.forEach { pitanja -> pitanja.anketaId = idAnkete }
                    AppDatabase.getInstance().pitanjeDao().insertAllEntities(pitanja)
                ...
                return@withContext try {
                    AppDatabase.getInstance().pitanjeDao().getByAnketumId(idAnkete)
                } catch (e: Exception) {
                    emptyList()
                }
            }
